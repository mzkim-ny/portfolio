...
- **제언**: ID당 주문이 단 한 건이어도, S사이즈를 먼저 선택하지 않는 것으로 보여집니다. 주문비율과 매출비율 모두 L사이즈가 가장 높고, M사이즈, S사이즈 순입니다. 사이즈별로 선호하는 메뉴가 다른만큼, S사이즈를 가장 많이 주문하는 목요일 12시에, 인기 메뉴 한정 쿠폰을 발행하는 이벤트를 제안합니다.
