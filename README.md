...
- **목적**: 1인 피자 시장이 커짐에 따라, 할인 쿠폰 발행 시 수요 변화를 참고하기 위함입니다.
...
- **시각화**: '사이즈별 매출비율'을 진행 그래프로 표현하였으며, '사이즈/시간/요일별 주문 건수'는 하이라이트 테이블로, '가장 많이 구매한 메뉴 5가지'는 트리맵 차트로 시각화하였습니다. **([태블로보기](https://public.tableau.com/shared/4FKB8ZMXH?:display_count=n&:origin=viz_share_link))**
...
- **목적**: 남양주 시민의 반려동물(개) 양육 현황을 대시보드로 파악하기 위함입니다.
