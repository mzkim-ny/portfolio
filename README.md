...
- **코드**: 국가별로 나눠진 세 개의 테이블을 하나의 테이블로 합쳐서 필요한 칼럼을 추출하였으며, 칼럼에 값이 없을 경우 문자를 입력하였습니다.
- **대시보드**: 국가별 추이(전체영화, 한국영화, 외국영화)가 어떠한 변화를 보이는지, 년도 범위를 선택하여 '매출액, 관객수, 영화수'를 그래프와 숫자로 확인할 수 있습니다.
